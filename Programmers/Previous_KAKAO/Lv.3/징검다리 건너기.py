...
def solution(stones, k):
    ...
    if len_stones == k:
        # k와 len_stones이 같으면 하나의 돌만 밟으면 되므로 최대값만큼 건널 수 있음.
        return max(stones)
    if all(stones[i-1] >= stones[i] for i in range(1, len_stones)):
    ...
    while i < len_stones - k:  # +1은 출발점. len_stones - k부터는 건너뛰면 도착.
        max_idx, max_stone = i+k, stones[i+k]
        for j in range(k-1, 0, -1):  # k는 위에서 처리했음. 출발 돌 제외해서 i+1까지만.
            if max_stone < stones[i+j]:
                max_idx, max_stone = i+j, stones[i+j]
...
"""
2019 카카오 개발자 겨울 인턴십
Lv.3. 현 시점 완료한 사람 7,955명, 정답률 48%
