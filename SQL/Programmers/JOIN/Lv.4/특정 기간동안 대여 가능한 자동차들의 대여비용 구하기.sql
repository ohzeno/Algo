...
*/
# WITH AVAILABE_CARS AS (SELECT CAR_ID, CAR_TYPE, DAILY_FEE AS FEE
...
#                        )),
#      DISCOUNTED_FEES AS (SELECT AC.CAR_ID,
#                                 AC.CAR_TYPE,
...
WITH CANT_RENT AS (SELECT CAR_ID
                   FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
     ...
     TOTAL_FEE AS (SELECT DAY.CAR_ID,
                          DAY.CAR_TYPE,
                          FLOOR(DAY.DAILY_FEE * 30 * (100 - DC.DISCOUNT_RATE) / 100) AS FEE
                   FROM CAR_RENTAL_COMPANY_CAR AS DAY
                            JOIN DISCOUNT AS DC
                                 ON DAY.CAR_TYPE = DC.CAR_TYPE
                   WHERE DAY.CAR_ID NOT IN (SELECT * FROM CANT_RENT))
...
/*
다시 풀면서 WITH문이 하나 늘었다. CAST 대신 FLOOR를 사용했고
금액에 대해 BETWEEN 대신 부등호를 사용했다.
가독성이 훨씬 올라간듯.
*/
