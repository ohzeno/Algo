...
1번 승객이 i번 자리에 앉는다면, i번 승객은 자신의 좌석이 1번인 취객인 상황으로 변한다.
...
3.  취객이 i번에 앉으면 n번 승객이 제자리에 앉을 확률은 Q(n-i+1)이 된다.

$$
{1 \over n} \cdot Q(n-i+1)
$$

3번 케이스는 i번 이전 승객은 제자리에 앉고, i번 부터 시작하는 상황으로 치환된다. 예를 들어 5명 중 1번 승객이 3번 자리에 앉았다면 3번 승객은 취객이 되어 1, 4, 5에 앉을 수 있으므로 Q(3)의 상황이 된다.

즉, Q(n)은 다음과 같다.

$$
\begin{align*}
Q(n) &= {1 \over n} \cdot 1 + {1 \over n} \cdot 0 + {1 \over n} \sum_{i=2}^{n-1} Q(n-i+1) \newline \newline &= {1 \over n} + {1 \over n}\sum_{i=2}^{n-1} Q(n-i+1)
\end{align*}
$$
...
뒤에서 x번째 승객의 자리는 y=100-x+1이다.

R은 y-1번째 승객까지가 1번, (100 - x + 1) ~ 100 좌석 중 100 - x + 1번을 선택하지 않았을 확률이 된다. 위에서 봤던 것과 같이 모든 무작위 선택에서 각 좌석이 선택될 확률은 균등하므로, x+1개의 좌석 중 y번이 선택되지 않을 확률은 다음과 같다.

$$
R(x) = {x \over {x+1}}, \quad 1 \le x \le 99
$$

## [변형 문제](https://www.acmicpc.net/problem/23337) 점화식

1번이 1번 자리에 앉지 못하는 경우 n번이 제자리에 앉을 확률을 S라 함.

1번이 1번 자리에 앉지 못하므로 S(1)은 존재하지 않음. n=2면 1번이 2번에 앉으므로 0.
$$
S(2) = 0
$$

S(3)부터 케이스가 둘이 됨. 1번이 n-1자리 중 선택하므로 분모는 n-1.

1. 1번 승객(취객)이 n번에 앉으면 n번 승객은 제자리에 앉을 수 없다.

$$
{1 \over n-1} \cdot 0
$$

2. 취객이 i번(2 <= i <= n-1)에 앉으면 n번 승객이 제자리에 앉을 확률은 Q(n-i+1)이 된다.

$$
{1 \over n-1} \cdot Q(n-i+1)
$$

즉, S(n)은 다음과 같다.
$$
\begin{align*}
S(n) &= {1 \over n-1} \cdot 0 + {1 \over n-1} \sum_{i=2}^{n-1} Q(n-i+1) \newline \newline &= {1 \over n-1}\sum_{i=2}^{n-1} Q(n-i+1)
\end{align*}
$$
변형 문제는 n번 승객이 제자리에 앉지 못할 확률을 구한다. 이를 F라 둔다.
$$
\begin{align*}
F(n) &= 1 - S(n)
\newline \newline &= 1 -{1 \over n-1}\sum_{i=2}^{n-1} Q(n-i+1)
\end{align*}
$$
