...
for _ in range(m):
    ...
    reversed_connects.setdefault(b, []).append((a, t))

def dijkstra(st, connects):
    dists = [float('inf')] * (n+1)
    q = [(0, st)]
    while q:
        sm_cost, mid = hpop(q)
...
backward = dijkstra(x, connects)
forward = dijkstra(x, reversed_connects)
ans = 0
...
우선순위큐를 사용하고 두번만 돌려봤지만 딱히 시간이 크게 차이나지 않았다.
"""